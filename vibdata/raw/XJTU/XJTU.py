...
class XJTU_raw(RawVibrationDataset, DownloadableDataset):
    ...
    resources = [("XJTU.zip", "7540900417abcee47603c986b9b04b48")]

    def __init__(self, root_dir: str, download=False):
        if download:
            super().__init__(
                root_dir=root_dir,
                download_resources=XJTU_raw.resources,
                download_urls=XJTU_raw.urls,
                extract_files=True,
            )
        else:
            super().__init__(root_dir=root_dir, download_resources=XJTU_raw.resources)

        self._metainfo = _get_package_resource_dataframe(__package__, "XJTU.csv")
    ...
    def custom_xjtu_filter(data: dict):
        ...
        metainfo = pd.DataFrame(metainfo.values.repeat(2, axis=0), columns=metainfo.columns)
