...
class SEU_raw(RawVibrationDataset, DownloadableDataset):
    ...
    def __init__(self, root_dir: str, download=False):
        if download:
            super().__init__(
                root_dir=root_dir,
                download_resources=SEU_raw.resources,
                download_urls=SEU_raw.mirrors,
                extract_files=True,
            )
        else:
    ...
    def __getitem__(self, i):
        ...
        if isinstance(i, list):
            ...
            for j in range(len(i)):
                ...
                channels = pd.read_csv(
                    ...
                    sep=sep,
                    skiprows=16,
                    names=["ch" + str(x) for x in range(1, 10)],
                ).values
                channels = channels[:, :8]
        ...
        channels = pd.read_csv(
            ...
            sep=sep,
            skiprows=16,
            names=["ch" + str(j) for j in range(1, 10)],
        ).values
        channels = channels[:, :8]
    ...
    def asSimpleForm(self):
        ...
        for f in filenames:
            ...
            channels = pd.read_csv(
                ...
                sep=sep,
                skiprows=16,
                names=["ch" + str(i) for i in range(1, 10)],
            )
